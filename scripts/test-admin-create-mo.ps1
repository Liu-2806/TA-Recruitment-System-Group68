param(
    [string]$JavaHome = "",
    [string]$WorkDirName = ".admin-create-mo-test",
    [string]$DataDir = ""
)
...
$sourceFiles = @(
    ...
    (Join-Path $projectRoot "src\com\bupt\ta\controller\admin\AdminMOListServlet.java")
)

& $javac -encoding UTF-8 -cp $servletApiJar -d $classesDir $sourceFiles
& $javac -encoding UTF-8 -cp "$classesDir;$servletApiJar" -d $runnerDir (Join-Path $projectRoot "scripts\AdminCreateMOConsoleRunner.java")
...
& $java -cp "$classesDir;$runnerDir;$servletApiJar" AdminCreateMOConsoleRunner

Write-Host ""
Write-Host "Created users files under: $resolvedDataDir\users"
