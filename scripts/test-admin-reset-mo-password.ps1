param(
    [string]$JavaHome = "",
    [string]$WorkDirName = ".admin-reset-mo-password-test",
    [string]$DataDir = ""
)
...
$sourceFiles = @(
    ...
    (Join-Path $projectRoot "src\com\bupt\ta\controller\admin\AdminMOCreateServlet.java"),
    (Join-Path $projectRoot "src\com\bupt\ta\controller\admin\AdminMOPasswordResetServlet.java")
)

& $javac -encoding UTF-8 -cp $servletApiJar -d $classesDir $sourceFiles
& $javac -encoding UTF-8 -cp "$classesDir;$servletApiJar" -d $runnerDir (Join-Path $projectRoot "scripts\AdminResetMOPasswordConsoleRunner.java")
...
& $java -cp "$classesDir;$runnerDir;$servletApiJar" AdminResetMOPasswordConsoleRunner

Write-Host ""
Write-Host "Validated password reset under: $resolvedDataDir\users\mo.json"
