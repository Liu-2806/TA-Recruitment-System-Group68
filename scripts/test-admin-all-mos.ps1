param(
    [string]$JavaHome = "",
    [string]$WorkDirName = ".admin-all-mos-test",
    [string]$DataDir = ""
)
...
& $javac -encoding UTF-8 -cp "$classesDir;$servletApiJar" -d $runnerDir (Join-Path $projectRoot "scripts\AdminAllMOsConsoleRunner.java")
...
& $java -cp "$classesDir;$runnerDir;$servletApiJar" AdminAllMOsConsoleRunner

Write-Host ""
Write-Host "Validated MO data under: $resolvedDataDir\users\mo.json"
